...
#End Region
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        strSql &= "AND t.name ='" & Me.txtTableName.Text & "' "
        ...
        ElseIf Me.rdBIS.Checked Then
            HandleAccessTable()
            Return
            'ElseIf Me.rdWholesale21.Checked Then
            '    oconn = GetWholeSale21Connection()
        ElseIf Me.rdOther.Checked Then
            If Me.txtOtherConnString.Text.Trim.Length > 0 Then
                oconn = GetOtherConnection(Me.txtOtherConnString.Text.Trim)
            Else
                MsgBox("please provide a valid connection string missed")
                Me.txtOtherConnString.Focus()
                Return
            End If
        Else
            oconn = Nothing
        ...
        mdt1 = New DataTable
        PctlLib.SqlDB.LoadDataTableBySqlCommand(mdt1, mycommand, oconn)



        Dim mDictColNameAndType As Dictionary(Of String, String) = New Dictionary(Of String, String)
        For i As Integer = 0 To mdt1.Rows.Count - 1
            'If i > 0 Then
            ...
            mDictColNameAndType.Add(mdt1.Rows(i).Item("COLUMN_NAME").ToString, mdt1.Rows(i).Item("SYSTEM_DATA_TYPE").ToString)
        Next

        Me.txtClassName.Clear()
        Me.txtClassType.Clear()
        For Each key As String In mDictColNameAndType.Keys

        Next


        Dim sbColName As New System.Text.StringBuilder
        Dim sbColType As New System.Text.StringBuilder
        For Each key As String In mDictColNameAndType.Keys
            sbColName.AppendLine(key)
            sbColType.AppendLine(mDictColNameAndType(key))
        Next

        Me.txtClassName.Text = sbColName.ToString 'sb to string will add one more empty line in text box
        Me.txtClassType.Text = sbColType.ToString
        Me.txtClassName.Text = Me.txtClassName.Text.Trim
        Me.txtClassType.Text = Me.txtClassType.Text.Trim
    End Sub
